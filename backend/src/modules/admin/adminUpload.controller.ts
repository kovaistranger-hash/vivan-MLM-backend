...
export async function adminUploadImage(req: Request, res: Response) {
  ...
  const file = req.file;
  if (!file?.buffer) throw new ApiError(400, 'No file uploaded (use multipart field name: file)');
  if (!file.mimetype.startsWith('image/')) throw new ApiError(400, 'Only image uploads are allowed');

  const folder = (req.body?.folder as string) || 'vivan/products';
