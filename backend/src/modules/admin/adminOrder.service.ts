...
const ORDER_STATUSES = ['pending', 'paid', 'packed', 'shipped', 'delivered', 'cancelled', 'returned'] as const;
export type OrderStatus = (typeof ORDER_STATUSES)[number];

export async function adminListOrders(params: { status?: string; page: number; pageSize: number }) {
...
export async function adminUpdateOrderStatus(
  id: number,
  status: OrderStatus,
  opts?: { refundToWallet?: boolean; adminUserId?: number }
) {
  ...
  try {
    ...
    const [ordRows]: any = await conn.query('SELECT * FROM orders WHERE id = ? FOR UPDATE', [id]);
    ...
    if (opts?.refundToWallet && (status === 'cancelled' || status === 'returned')) {
      ...
      let refundAmt = wAmt;
      if (String(order.payment_status) === 'paid') refundAmt = round2(refundAmt + net);
