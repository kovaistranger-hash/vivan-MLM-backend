...
async function istDateStrings(): Promise<{ today: string; yesterday: string }> {
  const rows = await query<Array<{ today: string; yesterday: string }>>(
    `SELECT DATE(CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+05:30')) AS today,
...
async function dailyProfitIst(day: string): Promise<number> {
  if (!day) return 0;
  const rev = await query<Array<{ s: string | number | null }>>(
  ...
  const pay = await query<Array<{ s: string | number | null }>>(
    `SELECT COALESCE(SUM(wallet_amount), 0) AS s
...
async function shouldInsertAlert(alertType: string): Promise<boolean> {
  const rows = await query<Array<{ c: number }>>(
    `SELECT COUNT(*) AS c FROM system_alerts
     WHERE alert_type = ? AND created_at > DATE_SUB(NOW(), INTERVAL ? MINUTE)`,
    [alertType, DEDUPE_MINUTES]
  );
  return Number(rows[0]?.c || 0) === 0;
}

async function insertAlert(row: RiskAlertRow): Promise<boolean> {
  if (!(await shouldInsertAlert(row.type))) return false;
  await execute(
    `INSERT INTO system_alerts (alert_type, severity, message, metadata_json)
     VALUES (?, ?, ?, ?)`,
    [row.type, row.severity, row.message, row.metadata != null ? JSON.stringify(row.metadata) : null]
  );
  return true;
}

async function applyBinaryMitigationIfNeeded(payoutRatio: number | null): Promise<boolean> {
  if (!env.riskAutoMitigate || payoutRatio == null || payoutRatio <= PAYOUT_DANGER) return false;
...
export async function listRecentSystemAlerts(limit = 30) {
  ...
  >(
    `SELECT id, alert_type, severity, message, metadata_json, created_at
     FROM system_alerts
     ORDER BY id DESC
     LIMIT :lim`,
    { lim }
 ...
 */
export async function evaluateSystemRiskAndPersist(): Promise<SystemRiskResult> {
  await ensureReferralSchemaExists();

  const revRow = await query<Array<{ s: string | number | null }>>(
    `SELECT COALESCE(SUM(subtotal), 0) AS s FROM orders WHERE status = 'delivered'`
  );
  const payRow = await query<Array<{ s: string | number | null }>>(
  ...
  const carryRow = await query<Array<{ L: string | number | null; R: string | number | null }>>(
    `SELECT COALESCE(SUM(left_profit_carry), 0) AS L, COALESCE(SUM(right_profit_carry), 0) AS R
     FROM binary_carry`
  );
  ...
  for (const a of alerts) {
    if (await insertAlert(a)) inserted++;
  }
