...
export type FraudIssue = { code: string; message: string; weight: number };
...
async function sponsorOf(userId: number): Promise<number | null> {
  const rows = await query<Array<{ sponsor_user_id: number | null }>>(
    'SELECT sponsor_user_id FROM referral_users WHERE user_id = :u LIMIT 1',
    { u: userId }
  );
...
async function hasSponsorCycle(userId: number): Promise<boolean> {
  const visited = new Set<number>();
  let cur: number | null = userId;
  for (let i = 0; i < 100; i++) {
    const sp = await sponsorOf(cur);
    if (sp == null) return false;
    if (sp === cur) return true;
    if (visited.has(sp)) return true;
    visited.add(cur);
    cur = sp;
  }
  return false;
...
async function shouldInsertFraudLog(userId: number, type: string): Promise<boolean> {
  const rows = await query<Array<{ c: number }>>(
    `SELECT COUNT(*) AS c FROM fraud_logs
     WHERE user_id = :u AND type = :t AND created_at > DATE_SUB(NOW(), INTERVAL ${DEDUPE_MINUTES} MINUTE)`,
    { u: userId, t: type }
  );
 ...
 */
export async function evaluateFraudRiskForUser(userId: number): Promise<FraudEvaluationResult> {
  ...
  if (hasSignupIp) {
    const ipRows = await query<Array<{ signup_ip: string | null }>>(
      'SELECT signup_ip FROM users WHERE id = :id LIMIT 1',
      { id: userId }
    );
    const ip = ipRows[0]?.signup_ip ? String(ipRows[0].signup_ip) : null;
    if (ip) {
      const cntRows = await query<Array<{ c: number }>>(
        'SELECT COUNT(*) AS c FROM users WHERE signup_ip = :ip',
        { ip }
      );
  ...
  const payDupRows = await query<Array<{ c: number }>>(
    `SELECT COUNT(DISTINCT ba.user_id) AS c
  ...
  const selfSponsor = await query<Array<{ c: number }>>(
    'SELECT COUNT(*) AS c FROM referral_users WHERE user_id = :u AND sponsor_user_id = :u',
    { u: userId }
  );
  ...
  const incomeRows = await query<Array<{ s: string | number | null }>>(
    `SELECT COALESCE(SUM(wallet_amount), 0) AS s FROM commission_transactions
     WHERE user_id = :u AND created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY)`,
    { u: userId }
  );
  ...
  const ordRows = await query<Array<{ c: number }>>(
    `SELECT COUNT(*) AS c FROM orders WHERE user_id = :u AND status IN ('paid','packed','shipped','delivered')`,
    { u: userId }
  );
  const ordCount = Number(ordRows[0]?.c || 0);
  if (ordCount === 0) {
    const commRows = await query<Array<{ s: string | number | null }>>(
      `SELECT COALESCE(SUM(wallet_amount), 0) AS s FROM commission_transactions WHERE user_id = :u`,
      { u: userId }
    );
  ...
  const dirRows = await query<Array<{ c: number }>>(
    'SELECT COUNT(*) AS c FROM referral_users WHERE sponsor_user_id = :u',
    { u: userId }
  );
  ...
  const wdRows = await query<Array<{ n: number; gross: string | number | null }>>(
    `SELECT COUNT(*) AS n, COALESCE(SUM(amount), 0) AS gross FROM withdrawal_requests
     WHERE user_id = :u AND status IN ('pending','approved','paid')
       AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)`,
    { u: userId }
  );
  const wdGross = Number(wdRows[0]?.gross ?? 0);
  const ageRows = await query<Array<{ days: number }>>(
    'SELECT DATEDIFF(NOW(), created_at) AS days FROM users WHERE id = :u LIMIT 1',
    { u: userId }
  );
  ...
  const wd24 = await query<Array<{ n: number; gross: string | number | null }>>(
    `SELECT COUNT(*) AS n, COALESCE(SUM(amount), 0) AS gross FROM withdrawal_requests
     WHERE user_id = :u AND created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY)`,
    { u: userId }
  );
  ...
  if (risk >= LOG_THRESHOLD) {
    ...
    if (await shouldInsertFraudLog(userId, type)) {
      ...
      await execute(
        `INSERT INTO fraud_logs (user_id, type, risk_score, message) VALUES (?, ?, ?, ?)`,
        [userId, type, risk, msg || 'Fraud heuristics triggered']
      );
      persisted = true;
    }
  }
  ...
  if (env.fraudAutoFreeze && risk >= FREEZE_THRESHOLD && (await columnExists('wallets', 'is_locked'))) {
    ...
    if (Number(r.affectedRows || 0) > 0) {
      ...
      if (await shouldInsertFraudLog(userId, 'AUTO_FREEZE')) {
        await execute(
          `INSERT INTO fraud_logs (user_id, type, risk_score, message) VALUES (?, 'AUTO_FREEZE', ?, ?)`,
          [userId, risk, 'Wallet locked automatically due to fraud score.']
        );
      }
    }
  }
...
export async function isWalletFraudLocked(userId: number): Promise<boolean> {
  if (!(await columnExists('wallets', 'is_locked'))) return false;
  const rows = await query<Array<{ is_locked: number }>>(
    'SELECT COALESCE(is_locked, 0) AS is_locked FROM wallets WHERE user_id = :u LIMIT 1',
    { u: userId }
  );
...
export async function adminListFraudLogs(limit = 80) {
  ...
  >(
    `SELECT f.id, f.user_id, f.type, f.risk_score, f.message, f.created_at, u.email, u.name
     FROM fraud_logs f
...
export async function adminFraudOverview() {
  ...
  const agg = await query<
    Array<{ user_id: number; max_score: number; email: string; name: string; last_at: Date | string }>
  >(
    `SELECT f.user_id, MAX(f.risk_score) AS max_score, u.email, u.name, MAX(f.created_at) AS last_at
     FROM fraud_logs f
     INNER JOIN users u ON u.id = f.user_id
