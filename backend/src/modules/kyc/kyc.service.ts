...
async function persistUpload(userId: number, label: string, buffer: Buffer, mimetype: string): Promise<string> {
  if (!buffer?.length) throw new ApiError(400, `Missing file: ${label}`);
...
export async function submitKycApplication(
  ...
  const pending = await query<Array<{ c: number }>>(
    `SELECT COUNT(*) AS c FROM kyc_records WHERE user_id = :u AND status = 'pending'`,
    { u: userId }
  );
  ...
  const ins = await execute(
    `INSERT INTO kyc_records (user_id, tier, pan_number, aadhaar_number, document_url, selfie_url, status)
     VALUES (?, ?, ?, ?, ?, ?, 'pending')`,
    [userId, tier, pan, maskAadhaar(aad12), docUrl, selfieUrl]
  );
  return { id: Number(ins.insertId || 0) };
}

export async function getKycStatusForUser(userId: number) {
  await ensureReferralSchemaExists();
  const urows = await query<Array<{ kyc_verified: number; kyc_level: number }>>(
    'SELECT COALESCE(kyc_verified,0) AS kyc_verified, COALESCE(kyc_level,0) AS kyc_level FROM users WHERE id = :u LIMIT 1',
    { u: userId }
  );
  const u = urows[0];
  ...
  >(
    `SELECT id, tier, status, rejection_reason, verified_at, created_at
     FROM kyc_records WHERE user_id = :u ORDER BY id DESC LIMIT 1`,
    { u: userId }
  );
  const latest = rec[0];
  return {
...
export async function hasVerifiedKyc(userId: number): Promise<boolean> {
  const s = await getKycStatusForUser(userId);
  if (s.userKycVerified) return true;
  return s.latest?.status === 'verified';
}

export async function assertWithdrawalKycEligible(userId: number, gross: number): Promise<void> {
  await ensureReferralSchemaExists();

  const srows = await query<Array<{ kyc_required: number }>>(
    'SELECT COALESCE(kyc_required,0) AS kyc_required FROM withdrawal_settings WHERE id = 1 LIMIT 1'
  );
  ...
  const rec = await query<Array<{ status: string; tier: string }>>(
    `SELECT status, tier FROM kyc_records WHERE user_id = :u ORDER BY id DESC LIMIT 1`,
    { u: userId }
  );
  const latest = rec[0];
  ...
  if (env.kycHighWithdrawalInr > 0 && gross >= env.kycHighWithdrawalInr && !ok) {
    throw new ApiError(
      403,
      `KYC verification is required for withdrawals of ₹${env.kycHighWithdrawalInr.toLocaleString('en-IN')} or more.`
    );
  }
  ...
  if (env.kycTier2WithdrawalInr > 0 && gross >= env.kycTier2WithdrawalInr) {
    ...
    if (!tier2Ok) {
      throw new ApiError(
        403,
        `Full KYC (L2) is required for withdrawals of ₹${env.kycTier2WithdrawalInr.toLocaleString('en-IN')} or more.`
      );
    }
  }
}
...
export async function adminDecideKyc(input: {
  ...
  await ensureReferralSchemaExists();
  const rows = await query<Array<{ user_id: number; tier: string; status: string }>>(
    'SELECT user_id, tier, status FROM kyc_records WHERE id = :id LIMIT 1',
    { id: input.recordId }
  );
  const r = rows[0];
  if (!r) throw new ApiError(404, 'KYC record not found');
  if (r.status !== 'pending') throw new ApiError(409, 'This application was already processed');

  const uid = Number(r.user_id);
  if (input.status === 'rejected') {
    await execute(
      `UPDATE kyc_records SET status = 'rejected', rejection_reason = ?, verified_at = NULL
       WHERE id = ?`,
      [input.rejectionReason?.trim() || 'Rejected', input.recordId]
    );
  ...
  await execute(
    `UPDATE kyc_records SET status = 'verified', rejection_reason = NULL, verified_at = NOW() WHERE id = ?`,
    [input.recordId]
  );
