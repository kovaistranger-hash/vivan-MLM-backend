...
export async function restockOrderLines(orderId: number) {
  const items = await query<{ product_id: number | null; quantity: number }[]>(
...
export async function restockOrderLinesTransactional(orderId: number) {
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const [items]: any = await conn.query('SELECT product_id, quantity FROM order_items WHERE order_id = ?', [orderId]);
    for (const row of items as { product_id: number | null; quantity: number }[]) {
      if (!row.product_id) continue;
