...
import {
  adminGetReferralUserBundle,
  adminListBinaryDaily,
  adminListCommissions,
  adminListReferralUsers,
  buildBinaryTreePreview,
  getBinaryTreeD3ForUser
} from './referral.service.js';

export async function adminReferralBinaryTreeD3(req: Request, res: Response) {
...
export async function adminReferralsUsersList(req: Request, res: Response) {
...
export async function adminReferralsUserDetail(req: Request, res: Response) {
  const userId = Number(req.params.userId);
  const bundle = await adminGetReferralUserBundle(userId);
  if (!bundle) return res.status(404).json({ success: false, message: 'User not found' });
...
export async function adminCommissionsList(req: Request, res: Response) {
  const page = Math.max(1, Number(req.query.page) || 1);
...
export async function adminBinaryDailyList(req: Request, res: Response) {
  const page = Math.max(1, Number(req.query.page) || 1);
  const pageSize = Math.min(100, Math.max(1, Number(req.query.pageSize) || 30));
  const userId = req.query.userId ? Number(req.query.userId) : undefined;
...
export async function adminCommissionsRecalculate(req: Request, res: Response) {
