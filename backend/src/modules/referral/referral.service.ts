...
function referralCodeFromRowField(rc: unknown): string {
  if (rc == null) return '';
  if (Buffer.isBuffer(rc)) return rc.toString('utf8').trim();
  if (typeof rc === 'string') return rc.trim();
  if (typeof rc === 'bigint') return String(rc).trim();
  if (typeof rc === 'object') return '';
  return String(rc).trim();
}

function isBlankReferralCode(rc: unknown): boolean {
  return referralCodeFromRowField(rc) === '';
}

/** Positive integer id for `referral_users.user_id` (JWT `sub` may be number or numeric string). */
export function normalizeReferralUserId(userId: unknown): number | null {
...
export async function getUserIdByReferralCode(code: string): Promise<number | null> {
  ...
  const rows = await query<{ user_id: number }[]>(
    'SELECT user_id FROM referral_users WHERE referral_code = :code LIMIT 1',
    { code: c }
  );
  return rows[0] ? Number(rows[0].user_id) : null;
}

export async function generateUniqueReferralCode(c: PoolConnection): Promise<string> {
  for (let i = 0; i < 30; i++) {
    const code = randomReferralCode();
    const [dup]: any = await c.query('SELECT user_id FROM referral_users WHERE referral_code = ? LIMIT 1', [code]);
...
export async function isInSponsorDownline(
  c: PoolConnection,
  sponsorUserId: number,
  nodeUserId: number
): Promise<boolean> {
  if (nodeUserId === sponsorUserId) return true;
  const [rows]: any = await c.query(
    `SELECT 1 FROM referral_closure WHERE ancestor_user_id = ? AND descendant_user_id = ? AND depth >= 1 LIMIT 1`,
    [sponsorUserId, nodeUserId]
  );
  return !!rows[0];
}

/** True if `candidateParentId` is `operatorUserId` or a binary descendant (via placement_parent only). */
export async function isInOperatorBinarySubtree(
  c: PoolConnection,
  operatorUserId: number,
  candidateParentId: number
): Promise<boolean> {
  if (candidateParentId === operatorUserId) return true;
  const [rows]: any = await c.query(
    `WITH RECURSIVE bin_sub AS (
     ...
     SELECT 1 AS ok FROM bin_sub WHERE uid = ? LIMIT 1`,
    [operatorUserId, candidateParentId]
  );
  return !!rows[0];
}

export async function insertSelfClosure(c: PoolConnection, userId: number) {
  await c.query(
    `INSERT IGNORE INTO referral_closure (ancestor_user_id, descendant_user_id, depth, leg_side) VALUES (?, ?, 0, NULL)`,
    [userId, userId]
  );
}

/** Copy closure rows from parent to new user and link (parent → user). */
export async function extendClosureForNewMember(
  c: PoolConnection,
  userId: number,
  parentUserId: number,
  side: 'left' | 'right'
  ...
  await c.query(
    `INSERT INTO referral_closure (ancestor_user_id, descendant_user_id, depth, leg_side)
    ...
    [userId, parentUserId, side, parentUserId]
  );
...
export type RegisterReferralInput = {
  ...
  placementParentReferralCode?: string | null;
  placementSide?: 'left' | 'right' | null;
  /** Precomputed auto-placement from registration (e.g. `findPlacement` in auth). Skips a second BFS in `createReferralProfileForNewCustomer`. */
  autoPlacement?: { parent_id: number; position: 'left' | 'right' } | null;
};

export async function createReferralProfileForNewCustomer(
  c: PoolConnection,
  userId: number,
  ...
  if (input.sponsorReferralCode?.trim()) {
    ...
    if (!sponsorId) throw new ApiError(400, 'Invalid sponsor referral code');
    if (sponsorId === userId) throw new ApiError(400, 'Cannot sponsor yourself');
  }
  ...
  let level = 1;
  if (sponsorId) {
    const [slev]: any = await c.query('SELECT level FROM referral_users WHERE user_id = ? FOR UPDATE', [sponsorId]);
    ...
    if (explicitParentCode) {
      ...
      if (!placementParentId) throw new ApiError(400, 'Invalid placement parent referral code');
  ...
  await c.query(
    `INSERT INTO referral_users (user_id, referral_code, sponsor_user_id, level, placement_parent_user_id, placement_side)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [userId, code, sponsorId, level, placementParentId, placementParentId ? side : null]
  );
 ...
 */
export async function ensureReferralProfileExists(userId: number) {
  await ensureReferralSchemaExists();

  const uid = normalizeReferralUserId(userId);
  if (uid == null) return;

  const rows = await query<{ user_id: number; referral_code: unknown }[]>(
    'SELECT user_id, referral_code FROM referral_users WHERE user_id = :u LIMIT 1',
    { u: uid }
  );
...
export async function getReferralRow(userId: number) {
  const uid = normalizeReferralUserId(userId);
  if (uid == null) return null;

  await ensureReferralProfileExists(uid);
  let rows = await query<any[]>(REFERRAL_ROW_SQL, { u: uid });
  let row = rows[0];
  if (row && isBlankReferralCode(row.referral_code)) {
    ...
    rows = await query<any[]>(REFERRAL_ROW_SQL, { u: uid });
    row = rows[0];
  }
  return row || null;
}
...
export function emptyReferralProfile(userId: number) {
  return {
    user_id: userId,
    referral_code: '',
    referralCode: '',
...
export function toReferralProfileDto(row: Record<string, unknown> | null) {
  if (!row) return null;
  const num = (v: unknown) => {
...
export async function listDirectReferrals(userId: number) {
  await ensureReferralProfileExists(userId);
  return query<any[]>(
    `SELECT u.id, u.name, u.email, ru.referral_code, ru.created_at,
...
export async function getBinaryTreeChildren(userId: number) {
  return query<any[]>(
    `SELECT u.id, u.name, u.email, ru.referral_code, ru.placement_side
     FROM referral_users ru
     INNER JOIN users u ON u.id = ru.user_id
     WHERE ru.placement_parent_user_id = :p
     ORDER BY ru.placement_side ASC, u.id ASC`,
    { p: userId }
  );
}

/** Sponsor places a sponsored member who has no binary parent yet, under a valid leg of `parentUserId`. */
export async function placeBinaryUnderParent(
  operatorUserId: number,
  input: { childReferralCode: string; parentReferralCode: string; side: 'left' | 'right' }
) {
  ...
  const parentCode = String(input.parentReferralCode || '')
    .trim()
    .toUpperCase();
  if (!childCode || !parentCode) throw new ApiError(400, 'Referral codes required');
  ...
  try {
    ...
    if (!childId) throw new ApiError(400, 'Invalid member referral code');

    const parentId = await getUserIdByReferralCode(parentCode);
    if (!parentId) throw new ApiError(400, 'Invalid parent referral code');

    const [childRows]: any = await c.query(
      `SELECT user_id, sponsor_user_id, placement_parent_user_id, placement_side FROM referral_users WHERE user_id = ? FOR UPDATE`,
      [childId]
    );
    const child = childRows[0];
    if (!child) throw new ApiError(404, 'Member is not on file');

    if (Number(child.sponsor_user_id) !== operatorUserId) {
      throw new ApiError(403, 'You can only place members you personally sponsored');
    }
    if (!child.sponsor_user_id) throw new ApiError(400, 'Member has no sponsor');
    if (child.placement_parent_user_id != null) {
      throw new ApiError(400, 'Member is already placed in the binary tree');
    }
    if (childId === parentId) throw new ApiError(400, 'Invalid placement');

    if (!(await isInOperatorBinarySubtree(c, operatorUserId, parentId))) {
      throw new ApiError(
        400,
        'Placement parent must be you or someone already under your binary tree (your left/right legs).'
      );
    }

    const [dup]: any = await c.query(
      `SELECT user_id FROM referral_users WHERE placement_parent_user_id = ? AND placement_side = ? LIMIT 1`,
      [parentId, input.side]
    );
    if (dup[0]) throw new ApiError(400, 'That position is already filled');

    await c.query(
      `UPDATE referral_users SET placement_parent_user_id = ?, placement_side = ? WHERE user_id = ?`,
      [parentId, input.side, childId]
    );
...
export function emptyBinaryTreeRoot(rootUserId: number) {
  return {
    id: rootUserId,
    name: '',
    email: '',
...
export async function buildBinaryTreePreview(rootUserId: number, maxDepth: number) {
  await ensureReferralProfileExists(rootUserId);

  async function node(uid: number, depth: number): Promise<any> {
    const urows = await query<any[]>(
      `SELECT u.id, u.name, u.email, ru.referral_code FROM users u
       INNER JOIN referral_users ru ON ru.user_id = u.id WHERE u.id = :id LIMIT 1`,
      { id: uid }
    );
...
export async function listCommissionsForUser(userId: number, page: number, pageSize: number) {
  await ensureReferralProfileExists(userId);
  ...
  const countRows = await query<{ total: number }[]>(
    `SELECT COUNT(*) AS total FROM commission_transactions WHERE user_id = :u`,
    { u: userId }
  );
...
export async function getBinarySummaryForUser(userId: number) {
  await ensureReferralProfileExists(userId);
  return mlmBinarySummaryPayload(userId);
}

export async function listWalletCommissionImpact(userId: number, limit = 30) {
  await ensureReferralProfileExists(userId);
  return query<any[]>(
...
export async function adminListReferralUsers(params: { q?: string; page: number; pageSize: number }) {
  await ensureReferralSchemaExists();
  ...
  const countRows = await query<{ total: number }[]>(
    `SELECT COUNT(*) AS total FROM referral_users ru INNER JOIN users u ON u.id = ru.user_id WHERE ${where.join(' AND ')}`,
    countVals
  );
  return { items, total: Number(countRows[0]?.total || 0) };
}

export async function adminGetReferralUserBundle(userId: number) {
  const profile = await getReferralRow(userId);
  if (!profile) return null;
  const directs = await listDirectReferrals(userId);
  const carryRows = await query<any[]>(
    `SELECT * FROM binary_carry WHERE user_id = :u LIMIT 1`,
    { u: userId }
  );
  const closureSample = await query<any[]>(
    `SELECT ancestor_user_id, depth, leg_side FROM referral_closure WHERE descendant_user_id = :u AND depth BETWEEN 1 AND 4 ORDER BY depth ASC`,
    { u: userId }
  );
...
export async function adminListCommissions(params: {
  ...
  const items = await query<any[]>(
    `SELECT ct.*, u.email AS user_email, u.name AS user_name, o.order_number
     FROM commission_transactions ct
  ...
  const { limit, offset: _o, ...countVals } = vals;
  const countRows = await query<{ total: number }[]>(
    `SELECT COUNT(*) AS total FROM commission_transactions ct WHERE ${w}`,
    countVals
  );
  return { items, total: Number(countRows[0]?.total || 0) };
}

export async function adminListBinaryDaily(params: { userId?: number; from?: string; to?: string; page: number; pageSize: number }) {
  await ensureReferralSchemaExists();
  ...
  const { limit, offset: _o, ...countVals } = vals;
  const countRows = await query<{ total: number }[]>(
    `SELECT COUNT(*) AS total FROM binary_daily_summary b WHERE ${w}`,
    countVals
  );
  return { items, total: Number(countRows[0]?.total || 0) };
}
...
function sanitizeUserIds(ids: number[]): number[] {
  return [...new Set(ids.map((i) => Math.trunc(Number(i))).filter((i) => Number.isFinite(i) && i > 0))];
}

function collectBinaryTreeUserIds(n: any, acc: Set<number>) {
  if (!n || n.id == null) return;
  acc.add(Number(n.id));
  if (n.left) collectBinaryTreeUserIds(n.left, acc);
  if (n.right) collectBinaryTreeUserIds(n.right, acc);
}

async function loadD3NodeStatsMap(userIds: number[]): Promise<Map<number, D3Stat>> {
  const map = new Map<number, D3Stat>();
  ...
  const [rts, carries, comm, levels] = await Promise.all([
    query<any[]>(
      `SELECT user_id, total_left_bv, total_right_bv, total_left_members, total_right_members
       FROM referral_tree_stats WHERE user_id IN (${ph})`,
      ids
    ),
    query<any[]>(`SELECT user_id, left_profit_carry, right_profit_carry FROM binary_carry WHERE user_id IN (${ph})`, ids),
    query<any[]>(
      `SELECT user_id,
       ...
       FROM commission_transactions WHERE user_id IN (${ph}) GROUP BY user_id`,
      ids
    ),
    query<any[]>(`SELECT user_id, level FROM referral_users WHERE user_id IN (${ph})`, ids)
  ]);
...
async function previewNodeToD3(n: any, stats: Map<number, D3Stat>, leg: string): Promise<any> {
  const id = Number(n.id);
  const s = stats.get(id) || { leftBV: 0, rightBV: 0, carryL: 0, carryR: 0, bin: 0, dir: 0, teamL: 0, teamR: 0, level: 1 };
  let weakLeg = 'balanced';
  if (s.carryL < s.carryR) weakLeg = 'left_carry';
  else if (s.carryR < s.carryL) weakLeg = 'right_carry';
  else if (s.leftBV < s.rightBV) weakLeg = 'left_bv';
  else if (s.rightBV < s.leftBV) weakLeg = 'right_bv';

  const children: any[] = [];
  if (!n.childrenTruncated) {
    if (n.left) children.push(await previewNodeToD3(n.left, stats, 'left'));
    if (n.right) children.push(await previewNodeToD3(n.right, stats, 'right'));
  }
...
export async function getBinaryTreeD3ForUser(rootUserId: number, maxDepth: number) {
  await ensureReferralSchemaExists();
