...
export async function createComplaint(userId: number, message: string) {
...
export async function adminListComplaints(params: {
  ...
  const countRows = await query<{ total: number }[]>(
    `SELECT COUNT(*) AS total FROM complaints c WHERE ${w}`,
    countVals
  );
  return { items, total: Number(countRows[0]?.total ?? 0) };
}

export async function adminUpdateComplaintStatus(complaintId: number, status: 'open' | 'resolved') {
  await ensureReferralSchemaExists();
  const rows = await query<Array<{ id: number }>>(`SELECT id FROM complaints WHERE id = :id LIMIT 1`, {
    id: complaintId
  });
  if (!rows[0]) throw new ApiError(404, 'Complaint not found');
