...
export async function adminWithdrawalsList(req: Request, res: Response) {
...
export async function adminWithdrawalGet(req: Request, res: Response) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) throw new ApiError(400, 'Invalid id');
  const data = await withdrawalService.adminGetWithdrawal(id);
  if (!data) throw new ApiError(404, 'Withdrawal not found');
  res.json({ success: true, ...data });
}

export async function adminWithdrawalApprove(req: Request, res: Response) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) throw new ApiError(400, 'Invalid id');
  const body = (req as any).validatedBody as { remarks?: string | null };
  const data = await withdrawalService.adminApproveWithdrawal(id, req.user!.id, body.remarks);
  res.json({ success: true, ...data });
}

export async function adminWithdrawalReject(req: Request, res: Response) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) throw new ApiError(400, 'Invalid id');
  const body = (req as any).validatedBody as { remarks?: string | null };
...
export async function adminWithdrawalMarkPaid(req: Request, res: Response) {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) throw new ApiError(400, 'Invalid id');
  const body = (req as any).validatedBody as { remarks?: string | null };
...
export async function adminWithdrawalSettingsGet(_req: Request, res: Response) {
...
export async function adminWithdrawalSettingsPut(req: Request, res: Response) {
  const body = (req as any).validatedBody as {
    min_withdrawal_amount: number;
    max_withdrawal_amount: number | null | undefined;
