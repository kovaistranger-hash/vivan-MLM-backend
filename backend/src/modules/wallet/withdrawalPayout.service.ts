 ...
 */
export async function onWithdrawalMarkedPaid(withdrawalId: number) {
  await ensureReferralSchemaExists();
  const rows = await query<any[]>(
    `SELECT wr.id, wr.user_id, wr.net_amount, wr.status FROM withdrawal_requests wr WHERE wr.id = :id LIMIT 1`,
    { id: withdrawalId }
  );
  const wr = rows[0];
  if (!wr || String(wr.status) !== 'paid') return;

  const existing = await query<Array<{ id: number }>>(
    `SELECT id FROM payouts WHERE withdrawal_request_id = :wid LIMIT 1`,
    { wid: withdrawalId }
  );
  ...
  const ins = await execute(
    `INSERT INTO payouts (withdrawal_request_id, user_id, amount, status, reference_id, provider)
     VALUES (:wid, :uid, :amt, 'pending', NULL, 'razorpay')`,
    { wid: withdrawalId, uid: userId, amt: net }
  );
  ...
  try {
    ...
    const j = (await r.json().catch(() => ({}))) as { id?: string; error?: { description?: string } };
    if (!r.ok) {
