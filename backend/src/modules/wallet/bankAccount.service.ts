...
export async function listBankAccounts(userId: number) {
  return query<any[]>(
    `SELECT * FROM bank_accounts WHERE user_id = :u ORDER BY is_default DESC, id DESC`,
    { u: userId }
  );
}
...
export async function getBankAccount(userId: number, id: number) {
  const rows = await query<any[]>(
    `SELECT * FROM bank_accounts WHERE id = :id AND user_id = :u LIMIT 1`,
    { id, u: userId }
  );
  return rows[0] || null;
}

async function clearDefaultExcept(c: PoolConnection, userId: number, exceptId: number | null) {
...
export async function createBankAccount(
  ...
  try {
    ...
    const [count]: any = await c.query('SELECT COUNT(*) AS n FROM bank_accounts WHERE user_id = ?', [userId]);
    const isFirst = Number(count[0]?.n ?? 0) === 0;
    const wantsDefault = isFirst || !!input.is_default;
    if (wantsDefault) await clearDefaultExcept(c, userId, null);
...
export async function updateBankAccount(
  ...
  try {
    ...
    if (input.is_default === true) nextDefault = 1;
    else if (input.is_default === false) nextDefault = 0;
    if (nextDefault === 1) await clearDefaultExcept(c, userId, id);
...
export async function deleteBankAccount(userId: number, id: number) {
  const [ref]: any = await pool.query(
    `SELECT id FROM withdrawal_requests WHERE bank_account_id = ? AND status IN ('pending','approved') LIMIT 1`,
    [id]
  );
  if (ref[0]) throw new ApiError(400, 'Cannot delete bank account linked to an open withdrawal');

  const r = await executeDelete(userId, id);
  if (!r) throw new ApiError(404, 'Bank account not found');
}

async function executeDelete(userId: number, id: number) {
  const [res]: any = await pool.query('DELETE FROM bank_accounts WHERE id = ? AND user_id = ?', [id, userId]);
