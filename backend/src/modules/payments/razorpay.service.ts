...
export async function createRazorpayOrderForLocalOrder(localOrderId: number, userId: number) {
  ...
  if (order.payment_method !== 'online') throw new ApiError(400, 'Order is not an online payment order');
  if (order.payment_status === 'paid') throw new ApiError(400, 'Order already paid');

  const amountPaise = rupeesToPaise(order.total_amount);
  if (amountPaise < 100) throw new ApiError(400, 'Order amount too small for Razorpay');
...
export async function verifyAndCaptureLocalOrder(input: {
  ...
  const payRows = await query<any[]>(
    `SELECT * FROM payments WHERE order_id = :orderId AND provider_order_id = :rzpOrder ORDER BY id DESC LIMIT 1`,
    { orderId: input.localOrderId, rzpOrder: input.razorpayOrderId }
  );
  const paymentRow = payRows[0];
  if (!paymentRow) throw new ApiError(400, 'Payment intent not found for this order');

  const expectedPaise = rupeesToPaise(order.total_amount);
  if (Number(paymentRow.amount_paise) !== expectedPaise) throw new ApiError(400, 'Amount mismatch');
  ...
  await execute(
    `UPDATE orders SET payment_status = 'paid', status = 'paid', updated_at = CURRENT_TIMESTAMP WHERE id = :id AND payment_status <> 'paid'`,
    { id: input.localOrderId }
  );
...
export async function handleRazorpayWebhookEvent(payload: any) {
  ...
  if (event === 'payment.captured') {
    ...
    if (!rzpPaymentId || !rzpOrderId) return;

    const pays = await query<any[]>(
      `SELECT p.id AS payment_id, p.order_id AS local_order_id, p.amount_paise, o.payment_status, o.total_amount
       FROM payments p
    ...
    await execute(
      `UPDATE payments SET provider_payment_id = :pid, status = 'captured', raw_payload_json = :raw, updated_at = CURRENT_TIMESTAMP WHERE id = :id`,
      { pid: rzpPaymentId, raw: JSON.stringify(payload), id: row.payment_id }
    );
    await execute(
      `UPDATE orders SET payment_status = 'paid', status = 'paid', updated_at = CURRENT_TIMESTAMP WHERE id = :id AND payment_status <> 'paid'`,
  ...
  if (event === 'payment.failed') {
    ...
    const pays = await query<any[]>(
      `SELECT p.id AS payment_id, p.order_id AS local_order_id, o.payment_status
       FROM payments p INNER JOIN orders o ON o.id = p.order_id
       WHERE p.provider_order_id = :rzpOrder ORDER BY p.id DESC LIMIT 1`,
      { rzpOrder: rzpOrderId }
    );
    const row = pays[0];
    if (!row || row.payment_status === 'paid') return;
    await execute(
      `UPDATE payments SET status = 'failed', raw_payload_json = :raw, updated_at = CURRENT_TIMESTAMP WHERE id = :id`,
      { raw: JSON.stringify(payload), id: row.payment_id }
    );
    await execute(
      `UPDATE orders SET payment_status = 'failed', updated_at = CURRENT_TIMESTAMP WHERE id = :id AND payment_status = 'pending'`,
      { id: row.local_order_id }
    );
