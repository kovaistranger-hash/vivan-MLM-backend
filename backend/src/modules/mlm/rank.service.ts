...
export function rankForTotalBv(totalBv: number): string {
  const v = Number(totalBv);
  if (!Number.isFinite(v) || v < 0) return 'Starter';
  if (v >= 5_000_000) return 'Diamond';
  if (v >= 1_000_000) return 'Platinum';
  if (v >= 200_000) return 'Gold';
  if (v >= 50_000) return 'Silver';
  return 'Starter';
}
...
export async function updateUserRank(
  ...
  if (!Number.isFinite(userId) || userId <= 0) return 'Starter';
