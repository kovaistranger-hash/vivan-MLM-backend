...
export function zeroBinarySummaryPayload() {
  return {
    leftCarry: 0,
    rightCarry: 0,
    carryUpdatedAt: null as string | null,
    todayBinaryPaid: 0,
...
export async function getBinarySummaryForUser(userId: number) {
  ...
  const carryRows = await query<any[]>(
    `SELECT left_profit_carry, right_profit_carry, updated_at FROM binary_carry WHERE user_id = :u LIMIT 1`,
    { u: userId }
  );
  const carry = carryRows[0] || { left_profit_carry: 0, right_profit_carry: 0, updated_at: null };
  const indiaDateRows = await query<{ d: string }[]>(
    `SELECT DATE(CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+05:30')) AS d`
  );
  const d = indiaDateRows[0]?.d || '';
  const dailyRows = await query<any[]>(
    `SELECT binary_paid_total, ceiling_blocked_total, updated_at FROM binary_daily_summary WHERE user_id = :u AND summary_date = :d LIMIT 1`,
    { u: userId, d }
  );
  ...
  const sums = await query<any[]>(
    `SELECT
     ...
     FROM commission_transactions WHERE user_id = :u`,
    { u: userId }
  );
  const w = sums[0] || {};
  return {
