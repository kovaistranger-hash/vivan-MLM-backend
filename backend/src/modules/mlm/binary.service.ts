...
export type ProcessBinaryResult = {
  usersProcessed: number;
  skippedSettings: boolean;
};

export type ProcessBinaryPayoutSummary = {
  eligible: number;
  paid: number;
  skippedAlreadyToday: number;
  skippedNoRoomOrZero: number;
  errors: number;
  skippedSettings: boolean;
};

export type ProcessBinaryPayoutOptions = {
  /** Keyset batch size for scanning `binary_carry` (default 1000, max 5000). */
  batchSize?: number;
};

type BinaryPayoutUserResult = 'paid' | 'skipped_already' | 'skipped_no_room' | 'skipped_settings' | 'error';
...
/** Audit shape for legacy `binary_match` / standstill reversals (order_id may be null). */
export type BinaryMatchAuditEvent = {
  ancestorUserId: number;
  matchedProfit: number;
  grossCommission: number;
  walletPaid: number;
  ceilingBlocked: number;
  summaryDate: string;
};
 ...
 */
export async function settleBinaryForAncestor(
  c: PoolConnection,
  ancestorUserId: number,
  orderId: number | null,
  orderNumber: string,
  summaryDate: string,
  matchEvents: BinaryMatchAuditEvent[],
  s: CompensationSettingsRow,
  snap: string
) {
  if (!s.binary_income_enabled || !s.carry_forward_enabled) return;
  ...
  const [rows]: any = await c.query(
    'SELECT left_profit_carry, right_profit_carry FROM binary_carry WHERE user_id = ? FOR UPDATE',
    [ancestorUserId]
  );
  if (!rows[0]) return;
  ...
  const [sumRows]: any = await c.query(
    `SELECT binary_paid_total FROM binary_daily_summary WHERE user_id = ? AND summary_date = ? FOR UPDATE`,
    [ancestorUserId, summaryDate]
  );
  ...
  await c.query(
    `UPDATE binary_carry SET
     ...
     WHERE user_id = ?`,
    [matched, matched, ancestorUserId]
  );
  ...
  if (payNow > 0) {
    ...
    await c.query(
      `UPDATE binary_daily_summary SET binary_paid_total = binary_paid_total + ?, updated_at = CURRENT_TIMESTAMP
       WHERE user_id = ? AND summary_date = ?`,
      [payNow, ancestorUserId, summaryDate]
    );
    await c.query(
  ...
  if (blocked > 0 && !discardCeiling) {
    ...
    await c.query(
      `UPDATE binary_daily_summary SET ceiling_blocked_total = ceiling_blocked_total + ?, updated_at = CURRENT_TIMESTAMP
       WHERE user_id = ? AND summary_date = ?`,
      [blocked, ancestorUserId, summaryDate]
    );
  }
...
async function ensureBinaryDailyRow(c: PoolConnection, userId: number, summaryDate: string) {
  await c.query(
    `INSERT IGNORE INTO binary_daily_summary (user_id, summary_date, binary_paid_total, ceiling_blocked_total)
     VALUES (?, ?, 0, 0)`,
    [userId, summaryDate]
  );
}
 ...
 */
async function runBinaryPayoutForUser(userId: number): Promise<BinaryPayoutUserResult> {
  if (!Number.isFinite(userId) || userId <= 0) return 'skipped_no_room';
  ...
  try {
    ...
    const [bcRows]: any = await c.query(
      `SELECT left_profit_carry, right_profit_carry FROM binary_carry WHERE user_id = ? FOR UPDATE`,
      [userId]
    );

    const [dup]: any = await c.query(
      `SELECT id FROM binary_payout_logs WHERE user_id = ? AND payout_date = ? LIMIT 1`,
      [userId, payoutDate]
    );
    if (dup[0]) {
      await c.commit();
    ...
    const [paidLogRows]: any = await c.query(
      `SELECT COALESCE(SUM(net_wallet_amount), 0) AS paid FROM binary_payout_logs WHERE user_id = ? AND payout_date = ?`,
      [userId, payoutDate]
    );
    ...
    void sendExpoPushToUser(
      userId,
      'Income credited 💰',
      `Binary income ₹${net} added to your wallet (${payoutDate}).`
    ).catch(() => undefined);
 ...
 */
export async function processBinaryPayout(opts?: ProcessBinaryPayoutOptions): Promise<ProcessBinaryPayoutSummary> {
  await ensureReferralSchemaExists();

  const summary: ProcessBinaryPayoutSummary = {
    eligible: 0,
    paid: 0,
  ...
  let peek: CompensationSettingsRow;
  try {
  ...
  const [eligibleRow] = await query<Array<{ c: number }>>(
    `SELECT COUNT(*) AS c FROM binary_carry
  ...
  for (;;) {
    const rows = await query<Array<{ user_id: number }>>(
      `SELECT user_id FROM binary_carry
    ...
    for (const row of rows) {
      ...
      else if (r === 'skipped_already') summary.skippedAlreadyToday++;
      else if (r === 'skipped_settings') summary.skippedSettings = true;
      else if (r === 'error') summary.errors++;
      else summary.skippedNoRoomOrZero++;
    }
  }
 ...
 */
export async function processBinary(): Promise<ProcessBinaryResult> {
 ...
 */
export async function runBinaryStandstillForUser(userId: number) {
  await ensureReferralSchemaExists();
...
export async function adminBinaryCarryList(params: { q?: string; page: number; pageSize: number }) {
  ...
  const countRows = await query<{ total: number }[]>(
    `SELECT COUNT(*) AS total FROM users u INNER JOIN roles r ON r.id = u.role_id AND r.slug = 'customer' WHERE ${w}`,
    countVals
  );
