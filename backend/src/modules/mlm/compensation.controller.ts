...
import type { CompensationSettingsBody } from './compensation.schemas.js';
...
export async function adminGetCompensationSettings(_req: Request, res: Response) {
  const settings = await getCompensationSettings();
  res.json({ success: true, settings });
}

export async function adminPutCompensationSettings(req: Request, res: Response) {
  const body = req.body as CompensationSettingsBody;
...
export async function adminPostManualAdjustment(req: Request, res: Response) {
...
export async function adminListManualAdjustments(req: Request, res: Response) {
...
export async function adminBinaryCarryListController(req: Request, res: Response) {
  const page = Math.max(1, Number(req.query.page) || 1);
...
export async function adminBinaryCarryAdjust(req: Request, res: Response) {
  const userId = Number(req.params.userId);
  const body = req.body as { side: 'left' | 'right' | 'reset'; delta?: number; reason: string };
...
export async function adminBinaryCarryRecalculate(req: Request, res: Response) {
  const userId = Number(req.params.userId);
