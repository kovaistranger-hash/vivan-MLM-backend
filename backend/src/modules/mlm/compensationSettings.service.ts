...
export type CompensationSettingsRow = {
  id: number;
  welcome_bonus_enabled: number;
  welcome_bonus_amount: number;
  direct_income_enabled: number;
  direct_income_type: 'percentage' | 'fixed';
  direct_income_value: number;
  trigger_stage: 'paid' | 'delivered';
  max_direct_income_per_order: number | null;
  binary_income_enabled: number;
  binary_left_percentage: number;
  binary_right_percentage: number;
  /** Single % of matched carry (gross binary before fees/caps); source of truth for payout rate. */
  binary_percentage: number;
  carry_forward_enabled: number;
  daily_binary_ceiling: number;
  binary_ceiling_behavior: 'hold_unpaid' | 'discard_unpaid';
  refund_reversal_enabled: number;
  minimum_order_profit: number;
  updated_at: Date;
...
export function serializeSettingsSnapshot(row: CompensationSettingsRow): Record<string, unknown> {
  return {
...
function mapRow(r: any): CompensationSettingsRow {
  return {
    id: Number(r.id),
...
export async function getCompensationSettings(): Promise<CompensationSettingsRow> {
  const [rows]: any = await pool.query('SELECT * FROM compensation_settings WHERE id = ? LIMIT 1', [SETTINGS_ID]);
  if (!rows[0]) throw new ApiError(500, 'compensation_settings not seeded');
  return mapRow(rows[0]);
}

/** Lock row for update inside an existing transaction. */
export async function getCompensationSettingsForUpdate(c: PoolConnection): Promise<CompensationSettingsRow> {
  const [rows]: any = await c.query('SELECT * FROM compensation_settings WHERE id = ? FOR UPDATE', [SETTINGS_ID]);
  if (!rows[0]) throw new ApiError(500, 'compensation_settings not seeded');
  return mapRow(rows[0]);
}

export type CompensationSettingsUpdateInput = {
  welcome_bonus_enabled: boolean;
  welcome_bonus_amount: number;
  direct_income_enabled: boolean;
  direct_income_type: 'percentage' | 'fixed';
  direct_income_value: number;
  trigger_stage: 'paid' | 'delivered';
  max_direct_income_per_order: number | null;
  binary_income_enabled: boolean;
  binary_left_percentage: number;
  binary_right_percentage: number;
  binary_percentage: number;
...
export async function updateCompensationSettings(input: CompensationSettingsUpdateInput, adminUserId: number) {
