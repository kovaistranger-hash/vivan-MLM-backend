 ...
 */
export async function dispatchOrderCommissionJob(orderId: number): Promise<{ mode: DispatchMode }> {
...
export async function dispatchBinaryCronJob(): Promise<{ mode: DispatchMode } | Awaited<ReturnType<typeof runDailyBinary>>> {
  const q = getCommissionQueue();
  if (!q) {
