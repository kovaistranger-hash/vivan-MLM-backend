...
export async function loadOrderForInvoice(orderId) {
    const rows = await query(`SELECT o.*, u.name AS customer_name, u.email AS customer_email
     FROM orders o
     INNER JOIN users u ON u.id = o.user_id
     WHERE o.id = :id LIMIT 1`, { id: orderId });
    const order = rows[0];
    if (!order)
        return null;
...
export async function persistOrderInvoiceToDisk(orderId) {
