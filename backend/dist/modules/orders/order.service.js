...
import { clearCheckoutIntentConn, debitWalletForOrder, getOrCreateWallet, getWalletBalance } from '../wallet/wallet.service.js';
function round2(n) {
...
export async function computeOrderPricing(userId, shippingPincode, wallet) {
    ...
    if (zonesConfigured) {
        if (!zoneRow)
            return { ok: false, message: 'We do not deliver to this PIN code.' };
...
export async function createOrder(userId, input) {
    ...
    try {
        ...
        const [wRows] = await conn.query('SELECT balance, checkout_intent_amount FROM wallets WHERE user_id = ? FOR UPDATE', [userId]);
        ...
        const { lines, merchandiseSubtotal, gstTotal, shippingAmount, discountAmount, walletApplied, payableTotal, codEnabled, codAllowedForPin, zonesConfigured } = priced;
        ...
        const [orderResult] = await conn.query(`INSERT INTO orders (
        ...
        for (const line of lines) {
            const [upd] = await conn.query('UPDATE products SET stock_qty = stock_qty - ? WHERE id = ? AND deleted_at IS NULL AND stock_qty >= ?', [line.qty, line.product_id, line.qty]);
            if (!upd.affectedRows) {
                throw new ApiError(400, `Insufficient stock for ${line.name}`);
            }
            await conn.query(`INSERT INTO order_items (order_id, product_id, product_name, product_slug, unit_price, gst_rate, line_gst, quantity, line_total)
         VALUES (?,?,?,?,?,?,?,?,?)`, [
                orderId,
                line.product_id,
                line.name,
        ...
        const orders = await query('SELECT * FROM orders WHERE id = :id LIMIT 1', { id: orderId });
        return orders[0];
    }
    catch (error) {
        if (inTx)
...
export async function listMyOrders(userId) {
    return query('SELECT * FROM orders WHERE user_id = :userId ORDER BY id DESC', { userId });
}
export async function getOrderByNumber(orderNumber) {
    const orders = await query('SELECT * FROM orders WHERE order_number = :orderNumber LIMIT 1', { orderNumber });
    if (!orders[0])
        return null;
    const items = await query(`SELECT id, product_id, product_name, product_slug, unit_price, gst_rate, line_gst, quantity, line_total
     FROM order_items WHERE order_id = :orderId`, { orderId: orders[0].id });
