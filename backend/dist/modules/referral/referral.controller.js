import { buildBinaryTreePreview, emptyBinaryTreeRoot, emptyReferralProfile, getBinarySummaryForUser, getReferralRow, listCommissionsForUser, listDirectReferrals, listWalletCommissionImpact, normalizeReferralUserId, placeBinaryUnderParent, toReferralProfileDto, zeroBinarySummaryPayload } from './referral.service.js';
...
function logReferralError(label, err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[referral] ${label}:`, msg);
}
export async function referralMe(req, res) {
    ...
    try {
        ...
        const flat = String(profile.referral_code ??
...
export async function referralTree(req, res) {
...
export async function referralDirects(req, res) {
    const uid = req.user.id;
    try {
...
export async function referralBinarySummary(req, res) {
    const uid = req.user.id;
    try {
...
export async function referralPlaceBinary(req, res) {
    const { childReferralCode, parentReferralCode, side } = req.body;
