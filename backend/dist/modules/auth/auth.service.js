...
export async function registerUser(input) {
    ...
    if (!customerRoleId)
        throw new ApiError(500, 'Roles not seeded');
    ...
    try {
        ...
        const [ins] = await c.query(`INSERT INTO users (role_id, email, password_hash, name, phone, signup_ip, is_active, accepted_terms)
       VALUES (?, ?, ?, ?, ?, ?, 1, ?)`, [customerRoleId, input.email, passwordHash, input.name, input.phone || null, input.signupIp || null, acceptedTerms]);
...
export async function refreshSession(refreshToken) {
    ...
    const rows = await query(`SELECT user_id, expires_at FROM refresh_tokens WHERE token_hash = :tokenHash LIMIT 1`, { tokenHash });
    const row = rows[0];
    if (!row)
    ...
    const users = await query(`SELECT u.*, r.slug AS role_slug
     FROM users u
     INNER JOIN roles r ON r.id = u.role_id
     WHERE u.id = :id
     LIMIT 1`, { id: row.user_id });
    const u = users[0];
    if (!u || !u.is_active)
...
async function issueAuthTokens(user, conn) {
    ...
    if (conn) {
        ...
        await execute(`INSERT INTO refresh_tokens (user_id, token_hash, expires_at) VALUES (:userId, :tokenHash, :expiresAt)`, { userId: user.id, tokenHash, expiresAt });
    }
    return {
