import { adminCreateProduct, adminGetProduct, adminListProducts, adminPatchProductFlags, adminSoftDeleteProduct, adminSuggestProductIdentifiers, adminUpdateProduct } from './adminProduct.service.js';
...
export async function adminProductsList(req, res) {
...
export async function adminProductGet(req, res) {
...
export async function adminProductCreate(req, res) {
    const id = await adminCreateProduct(normalizeProductBody(req.body));
    res.status(201).json({ success: true, id });
}
export async function adminProductUpdate(req, res) {
    const id = Number(req.params.id);
    await adminUpdateProduct(id, normalizeProductBody(req.body));
    res.json({ success: true });
}
export async function adminProductDelete(req, res) {
    await adminSoftDeleteProduct(Number(req.params.id));
    res.json({ success: true });
}
export async function adminProductPatchFlags(req, res) {
    const id = Number(req.params.id);
    await adminPatchProductFlags(id, req.body);
    res.json({ success: true });
}
export async function adminProductSuggestIdentifiers(req, res) {
    const q = req.validatedQuery;
