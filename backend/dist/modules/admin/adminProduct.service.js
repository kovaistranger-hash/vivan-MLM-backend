...
import { ensureUniqueSku, ensureUniqueSlug, generateSku, nextSkuSequenceForCategoryCode, resolveCategoryCodeForSku, slugifyName } from './productIdentifiers.js';
...
export async function adminListProducts(params) {
  ...
  `;
    ...
    const countRows = await query(`SELECT COUNT(*) AS total FROM products p WHERE ${where.join(' AND ')}`, countVals);
    const total = Number(countRows[0]?.total || 0);
    return { items, total, lowStockThreshold: LOW_STOCK_THRESHOLD };
}
export async function adminGetProduct(id) {
    const rows = await query(`SELECT p.*, c.name AS category_name, b.name AS brand_name
     FROM products p
     LEFT JOIN categories c ON c.id = p.category_id
     LEFT JOIN brands b ON b.id = p.brand_id
     WHERE p.id = :id LIMIT 1`, { id });
    return rows[0] || null;
}
...
export async function adminUpdateProduct(id, input) {
    ...
    if (existing.deleted_at)
        throw new ApiError(400, 'Product is archived');
...
export async function adminLowStockCount() {
    const rows = await query(`SELECT COUNT(*) AS c FROM products WHERE deleted_at IS NULL AND is_active = 1 AND stock_qty < :t`, { t: LOW_STOCK_THRESHOLD });
    return Number(rows[0]?.c || 0);
}
export async function adminPatchProductFlags(id, flags) {
    const existing = await adminGetProduct(id);
    if (!existing)
        throw new ApiError(404, 'Product not found');
    if (existing.deleted_at)
