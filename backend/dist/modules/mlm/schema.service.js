...
async function tableExists(tableName) {
    const rows = await query(`SELECT COUNT(*) AS total
     ...
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = :tableName`, { tableName });
    return Number(rows[0]?.total || 0) > 0;
}
async function indexExists(tableName, indexName) {
    const rows = await query(`SELECT COUNT(*) AS total
     FROM information_schema.STATISTICS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = :tableName AND INDEX_NAME = :indexName`, { tableName, indexName });
    return Number(rows[0]?.total || 0) > 0;
}
async function ensureBinaryCarryMatchableIndex() {
    if (!(await tableExists('binary_carry')))
        return;
    if (await indexExists('binary_carry', 'idx_binary_carry_matchable'))
        return;
    await query(`ALTER TABLE binary_carry ADD INDEX idx_binary_carry_matchable (left_profit_carry, right_profit_carry)`);
}
async function ensureCompensationBinaryPercentageColumn() {
    const tbl = await query(`SELECT COUNT(*) AS c FROM information_schema.TABLES
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'compensation_settings'`);
    if (!tbl[0] || Number(tbl[0].c) === 0)
        return;
    if (await columnExists('compensation_settings', 'binary_percentage'))
        return;
    await query(`ALTER TABLE compensation_settings
    ...
    await query(`UPDATE compensation_settings
...
async function ensureReferralUsersMlmColumns() {
    if (!(await columnExists('referral_users', 'level'))) {
        await query(`ALTER TABLE referral_users ADD COLUMN level INT NOT NULL DEFAULT 1 AFTER sponsor_user_id`);
    }
}
...
async function ensureKycAndScoreTables() {
  ...
  `);
    if (!(await columnExists('users', 'kyc_level'))) {
        await query(`ALTER TABLE users ADD COLUMN kyc_level TINYINT NOT NULL DEFAULT 0 COMMENT '0=L0,1=L1,2=L2' AFTER kyc_verified`);
    }
}
async function ensureIndiaComplianceArtifacts() {
    ...
    if (!(await columnExists('users', 'accepted_terms'))) {
        await query(`ALTER TABLE users ADD COLUMN accepted_terms TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Terms + policies accepted at registration' AFTER is_active`);
    }
...
async function ensureFraudArtifacts() {
  ...
  `);
    ...
    if (!(await columnExists('wallets', 'is_locked'))) {
        await query(`ALTER TABLE wallets ADD COLUMN is_locked TINYINT(1) NOT NULL DEFAULT 0 AFTER is_active`);
    }
}
async function ensureReferralUsersPlacementConstraints() {
    if (!(await tableExists('referral_users')))
        return;
    if (!(await indexExists('referral_users', 'uq_referral_placement_leg')) &&
        !(await indexExists('referral_users', 'unique_position'))) {
        await query(`ALTER TABLE referral_users
         ADD UNIQUE KEY unique_position (placement_parent_user_id, placement_side)`);
    }
    if (!(await indexExists('referral_users', 'idx_referral_placement_parent')) &&
...
async function ensureCleanArchitectureIndexes() {
    ...
    if (!(await columnExists('users', 'is_blocked'))) {
        await query(`ALTER TABLE users ADD COLUMN is_blocked TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Admin/system account block' AFTER is_active`);
    }
}
