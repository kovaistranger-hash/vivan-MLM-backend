...
export async function distributeBVConn(c, userId, bv) {
    ...
    if (!Number.isFinite(uid) || uid <= 0)
        return [];
    const [chainRows] = await c.query(`WITH RECURSIVE chain AS (
       SELECT
    ...
    await c.query(`INSERT IGNORE INTO binary_carry (user_id, left_profit_carry, right_profit_carry) VALUES ${placeholders}`, uniqIds);
    ...
    if (leftIds.length) {
       ...
       WHERE user_id IN (${ph})`, [creditedBv, ...leftIds]);
    }
    if (rightIds.length) {
        ...
        await c.query(`UPDATE binary_carry SET
         ...
         updated_at = CURRENT_TIMESTAMP
       WHERE user_id IN (${ph})`, [creditedBv, ...rightIds]);
    }
    return adds;
}
