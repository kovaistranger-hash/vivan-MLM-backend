...
export async function createManualAdjustment(input) {
    ...
    try {
        ...
        const [insAdj] = await c.query(`INSERT INTO commission_manual_adjustments (
        ...
        if (input.applyToWallet) {
            ...
            if (input.action === 'credit') {
                ...
                const [wrows] = await c.query(`SELECT id FROM wallet_transactions WHERE user_id = ? AND reference_type = 'manual_adjustment' AND reference_id = ? ORDER BY id DESC LIMIT 1`, [input.userId, adjId]);
                walletTxId = wrows[0] ? Number(wrows[0].id) : null;
            }
            const walletSigned = input.action === 'credit' ? amt : round2(-amt);
            const [insComm] = await c.query(`INSERT INTO commission_transactions (
...
export async function listManualAdjustments(params) {
    ...
    const items = await query(`SELECT m.*, u.email AS user_email, u.name AS user_name, a.email AS created_by_email
    ...
    const countRows = await query(`SELECT COUNT(*) AS total FROM commission_manual_adjustments m WHERE ${w}`, countVals);
    return { items, total: Number(countRows[0]?.total || 0) };
}
export async function createCarryAdjustment(c, input) {
    await c.query(`INSERT IGNORE INTO binary_carry (user_id) VALUES (?)`, [input.userId]);
    const [rows] = await c.query('SELECT left_profit_carry, right_profit_carry FROM binary_carry WHERE user_id = ? FOR UPDATE', [
        input.userId
    ]);
    if (!rows[0])
        throw new ApiError(404, 'User carry not found');
    ...
    await c.query(`UPDATE binary_carry SET left_profit_carry = ?, right_profit_carry = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?`, [left, right, input.userId]);
    await c.query(`INSERT INTO commission_manual_adjustments (
