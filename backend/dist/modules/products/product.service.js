...
export async function listProducts(params) {
    const where = ['p.is_active = 1', 'p.deleted_at IS NULL'];
  ...
  `;
    ...
    const countRows = await query(countSql, countValues);
    const total = Number(countRows[0]?.total || 0);
    return { items, total };
}
export async function autocompleteProducts(q, limit = 8) {
    if (!q.trim())
        return [];
    const storefrontCatClause = `EXISTS (
    SELECT 1 FROM categories _sf
    ...
    return query(`SELECT p.id, p.name, p.slug, p.sale_price, p.mrp_price, p.image_url
     FROM products p
     WHERE p.is_active = 1 AND p.deleted_at IS NULL AND (${storefrontCatClause}) AND (p.name LIKE :q OR p.sku LIKE :q)
     ORDER BY p.name ASC
     LIMIT :limit`, { q: `%${q}%`, limit });
}
export async function getProductBySlug(slug) {
    const rows = await query(`SELECT p.*, c.name AS category_name, c.slug AS category_slug, b.name AS brand_name
     FROM products p
     LEFT JOIN categories c ON c.id = p.category_id
     LEFT JOIN brands b ON b.id = p.brand_id
    ...
    if (!product)
        return null;
    const reviews = await query(`SELECT r.id, r.rating, r.title, r.body, r.created_at, u.name AS author_name
     FROM product_reviews r
     INNER JOIN users u ON u.id = r.user_id
     WHERE r.product_id = :productId AND r.is_approved = 1
     ORDER BY r.created_at DESC
     LIMIT 50`, { productId: product.id });
    const summary = await query(`SELECT AVG(rating) AS avg_rating, COUNT(*) AS review_count
     FROM product_reviews
     WHERE product_id = :productId AND is_approved = 1`, { productId: product.id });
    const storefrontCatClause = `EXISTS (
    SELECT 1 FROM categories _sf
    WHERE _sf.id = p.category_id AND _sf.is_active = 1
    AND _sf.slug IN (${storefrontCategorySlugSqlList()})
  )`;
    const related = await query(`SELECT p.id, p.name, p.slug, p.sale_price, p.mrp_price, p.image_url, p.bv, p.pv
     FROM products p
     ...
     LIMIT 6`, { categoryId: product.category_id, productId: product.id });
    return {
        ...product,
        reviews,
        reviewSummary: {
