...
export async function getWithdrawalSettings() {
    const [rows] = await pool.query('SELECT * FROM withdrawal_settings WHERE id = ? LIMIT 1', [SETTINGS_ID]);
    if (!rows[0])
        throw new ApiError(500, 'withdrawal_settings not seeded');
    return mapRow(rows[0]);
}
export async function getWithdrawalSettingsForUpdate(c) {
    const [rows] = await c.query('SELECT * FROM withdrawal_settings WHERE id = ? FOR UPDATE', [SETTINGS_ID]);
    if (!rows[0])
        throw new ApiError(500, 'withdrawal_settings not seeded');
    return mapRow(rows[0]);
}
export async function updateWithdrawalSettings(input, adminUserId) {
    const c = await pool.getConnection();
    try {
        await c.beginTransaction();
        await getWithdrawalSettingsForUpdate(c);
        await c.query(`UPDATE withdrawal_settings SET
...
export function computeWithdrawalFee(gross, s) {
