...
function assertBankMatchesSettings(acct, s) {
    ...
    let ok = false;
    if (s.allow_bank_transfer && bankPair)
        ok = true;
    if (s.allow_upi && upiOk)
        ok = true;
    if (!ok)
        throw new ApiError(400, 'This bank profile does not match allowed withdrawal methods');
}
export async function listUserWithdrawals(userId, page, pageSize) {
    const limit = pageSize;
    const offset = (page - 1) * limit;
    const items = await query(`SELECT wr.*, ba.bank_name, ba.account_holder_name, ba.account_number, ba.ifsc_code, ba.upi_id
     FROM withdrawal_requests wr
     INNER JOIN bank_accounts ba ON ba.id = wr.bank_account_id
     WHERE wr.user_id = :u
     ORDER BY wr.id DESC
     LIMIT :limit OFFSET :offset`, { u: userId, limit, offset });
    const countRows = await query(`SELECT COUNT(*) AS total FROM withdrawal_requests WHERE user_id = :u`, { u: userId });
...
export async function createWithdrawalRequest(userId, input) {
    ...
    if (fraud.riskScore >= FRAUD_WITHDRAW_BLOCK || (await isWalletFraudLocked(userId))) {
        throw new ApiError(403, 'Withdrawal is not available for this account pending compliance review. Please contact support.');
    }
    ...
    try {
        ...
        const [sRows] = await c.query('SELECT * FROM withdrawal_settings WHERE id = 1 FOR UPDATE');
        const s = sRows[0];
        if (!s)
            throw new ApiError(500, 'Withdrawal settings missing');
        const [uRows] = await c.query('SELECT id FROM users WHERE id = ? FOR UPDATE', [userId]);
        if (!uRows[0])
            throw new ApiError(404, 'User not found');
        const [bRows] = await c.query('SELECT * FROM bank_accounts WHERE id = ? AND user_id = ? FOR UPDATE', [input.bankAccountId, userId]);
        const bank = bRows[0];
        if (!bank)
        ...
        if (Number(s.one_pending_request_only) === 1) {
            const [open] = await c.query(`SELECT id FROM withdrawal_requests
         WHERE user_id = ? AND status IN ('pending','approved') LIMIT 1 FOR UPDATE`, [userId]);
            if (open[0])
                throw new ApiError(400, 'You already have an open withdrawal request');
        }
        ...
        if (net <= 0)
            throw new ApiError(400, 'Withdrawal amount too low after fees');
        const [ins] = await c.query(`INSERT INTO withdrawal_requests (
         user_id, bank_account_id, amount, fee_amount, net_amount, status, notes
       ) VALUES (?,?,?,?,?,'pending',?)`, [userId, input.bankAccountId, gross, fee, net, input.notes?.trim() || null]);
        const withdrawalId = Number(ins.insertId);
        await applyWithdrawalHold(c, userId, gross, withdrawalId);
        if (Number(s.auto_approve) === 1) {
            await c.query(`UPDATE withdrawal_requests SET status = 'approved', processed_by = NULL, processed_at = NOW(), updated_at = CURRENT_TIMESTAMP
...
export async function cancelWithdrawalRequest(userId, withdrawalId) {
    ...
    try {
        await c.beginTransaction();
        const [rows] = await c.query('SELECT * FROM withdrawal_requests WHERE id = ? AND user_id = ? FOR UPDATE', [withdrawalId, userId]);
        const wr = rows[0];
        if (!wr)
        ...
        await c.query(`UPDATE withdrawal_requests SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [withdrawalId]);
        await c.commit();
        return getWithdrawalByIdForUser(userId, withdrawalId);
    }
    catch (e) {
...
export async function getWithdrawalByIdForUser(userId, id) {
    const rows = await query(`SELECT wr.*, ba.bank_name, ba.account_holder_name, ba.account_number, ba.ifsc_code, ba.upi_id
     FROM withdrawal_requests wr
     INNER JOIN bank_accounts ba ON ba.id = wr.bank_account_id
     WHERE wr.id = :id AND wr.user_id = :u LIMIT 1`, { id, u: userId });
    return rows[0] || null;
}
export async function adminListWithdrawals(params) {
    ...
    const items = await query(`SELECT wr.*, u.email AS user_email, u.name AS user_name,
    ...
    const countRows = await query(`SELECT COUNT(*) AS total FROM withdrawal_requests wr WHERE ${w}`, countVals);
    return { items, total: Number(countRows[0]?.total ?? 0) };
}
export async function adminGetWithdrawal(id) {
    const rows = await query(`SELECT wr.*, u.email AS user_email, u.name AS user_name,
            ba.bank_name, ba.account_holder_name, ba.account_number, ba.ifsc_code, ba.upi_id, ba.branch_name
     FROM withdrawal_requests wr
     INNER JOIN users u ON u.id = wr.user_id
     INNER JOIN bank_accounts ba ON ba.id = wr.bank_account_id
     WHERE wr.id = :id LIMIT 1`, { id });
    if (!rows[0])
        return null;
    const wid = rows[0].user_id;
    await getOrCreateWallet(wid);
    const [wRows] = await pool.query('SELECT id, user_id, balance, held_balance, is_active, checkout_intent_amount FROM wallets WHERE user_id = ? LIMIT 1', [wid]);
    return { withdrawal: rows[0], wallet: wRows[0] || null };
}
export async function adminApproveWithdrawal(withdrawalId, adminUserId, remarks) {
    const c = await pool.getConnection();
    try {
        await c.beginTransaction();
        const [rows] = await c.query('SELECT * FROM withdrawal_requests WHERE id = ? FOR UPDATE', [withdrawalId]);
        const wr = rows[0];
        if (!wr)
            throw new ApiError(404, 'Withdrawal not found');
        if (wr.status !== 'pending')
            throw new ApiError(400, 'Only pending requests can be approved');
        const [updAp] = await c.query(`UPDATE withdrawal_requests SET
         status = 'approved',
         processed_by = ?,
         processed_at = NOW(),
         admin_remarks = COALESCE(?, admin_remarks),
         updated_at = CURRENT_TIMESTAMP
       WHERE id = ? AND status = 'pending'`, [adminUserId, remarks?.trim() || null, withdrawalId]);
...
export async function adminRejectWithdrawal(withdrawalId, adminUserId, remarks) {
    const c = await pool.getConnection();
    try {
        await c.beginTransaction();
        const [rows] = await c.query('SELECT * FROM withdrawal_requests WHERE id = ? FOR UPDATE', [withdrawalId]);
        ...
        const [upd] = await c.query(`UPDATE withdrawal_requests SET
         status = 'rejected',
         processed_by = ?,
         processed_at = NOW(),
         admin_remarks = ?,
         updated_at = CURRENT_TIMESTAMP
       WHERE id = ? AND status IN ('pending','approved')`, [adminUserId, remarks?.trim() || null, withdrawalId]);
...
export async function adminMarkWithdrawalPaid(withdrawalId, adminUserId, remarks) {
    const c = await pool.getConnection();
    try {
        await c.beginTransaction();
        const [rows] = await c.query('SELECT * FROM withdrawal_requests WHERE id = ? FOR UPDATE', [withdrawalId]);
        ...
        if (wr.status !== 'approved')
            throw new ApiError(400, 'Only approved requests can be marked paid');
        const gross = Number(wr.amount);
        const net = Number(wr.net_amount);
        const uid = Number(wr.user_id);
        await applyWithdrawalPaidRelease(c, uid, gross, withdrawalId, net);
        const [updPaid] = await c.query(`UPDATE withdrawal_requests SET
         status = 'paid',
         processed_by = ?,
         processed_at = NOW(),
         admin_remarks = COALESCE(?, admin_remarks),
         updated_at = CURRENT_TIMESTAMP
       WHERE id = ? AND status = 'approved'`, [adminUserId, remarks?.trim() || null, withdrawalId]);
