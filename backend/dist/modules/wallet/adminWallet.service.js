...
export async function adminListWallets(params) {
    ...
    const countRows = await query(`SELECT COUNT(*) AS total FROM users u
     INNER JOIN roles r ON r.id = u.role_id
     WHERE ${where.join(' AND ')}`, countVals);
    return { items, total: Number(countRows[0]?.total || 0) };
}
export async function adminGetWalletDetail(userId) {
    await getOrCreateWallet(userId);
    const walletRows = await query(`SELECT w.*, u.name AS user_name, u.email AS user_email
     FROM wallets w INNER JOIN users u ON u.id = w.user_id
     WHERE w.user_id = :u LIMIT 1`, { u: userId });
    if (!walletRows[0])
        throw new ApiError(404, 'Wallet not found for this user');
    const txs = await query(`SELECT * FROM wallet_transactions WHERE user_id = :u ORDER BY id DESC LIMIT 250`, { u: userId });
    return { wallet: walletRows[0], transactions: txs };
}
export async function adminCreditUserWallet(userId, adminId, amount, description) {
...
export async function adminDebitUserWallet(userId, adminId, amount, description) {
    const c = await pool.getConnection();
    try {
        await c.beginTransaction();
        await getOrCreateWallet(userId, c);
...
export async function adminUnlockWalletFraudLock(userId) {
    await query(`UPDATE wallets SET is_locked = 0, updated_at = CURRENT_TIMESTAMP WHERE user_id = :u`, { u: userId });
}
export async function adminManualRefundToWallet(userId, adminId, amount, description, referenceType, referenceId) {
    const c = await pool.getConnection();
    try {
        await c.beginTransaction();
        await getOrCreateWallet(userId, c);
...
export async function adminWalletDashboardStats() {
    const [bal, cred, deb] = await Promise.all([
        ...
        query(`SELECT COALESCE(SUM(amount),0) AS s FROM wallet_transactions WHERE type IN ('credit','admin_credit','refund','order_refund','commission_credit','bonus_credit')`),
        query(`SELECT COALESCE(SUM(amount),0) AS s FROM wallet_transactions WHERE type IN ('debit','admin_debit','order_payment')`)
    ]);
    const recent = await query(`SELECT t.*, u.email AS user_email, u.name AS user_name
