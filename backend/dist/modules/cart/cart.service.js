...
export async function getCart(userId) {
    const rows = await query(`SELECT ci.id, ci.quantity,
...
export async function addToCart(userId, productId, quantity) {
    const products = await query('SELECT id, stock_qty, is_active FROM products WHERE id = :productId AND deleted_at IS NULL LIMIT 1', { productId });
    const product = products[0];
    if (!product || !product.is_active)
        throw new ApiError(404, 'Product not found');
    if (product.stock_qty < quantity)
        throw new ApiError(400, 'Insufficient stock');
    const existing = await query('SELECT id, quantity FROM cart_items WHERE user_id = :userId AND product_id = :productId LIMIT 1', {
        userId,
        productId
    });
    if (existing[0]) {
        const nextQty = Number(existing[0].quantity) + quantity;
        if (product.stock_qty < nextQty)
            throw new ApiError(400, 'Insufficient stock');
...
export async function updateCartItem(userId, itemId, quantity) {
    ...
    const rows = await query(`SELECT ci.id, p.stock_qty
     FROM cart_items ci
     INNER JOIN products p ON p.id = ci.product_id AND p.deleted_at IS NULL
