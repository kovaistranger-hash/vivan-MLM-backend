...
const strictEnv = isProduction || ['1', 'true', 'yes'].includes(String(process.env.STRICT_ENV || '').toLowerCase());
...
function requiredStrict(name, devFallback) {
    if (strictEnv) {
...
if (strictEnv) {
    if (jwtAccessSecret.length < MIN_JWT_SECRET_LEN || jwtRefreshSecret.length < MIN_JWT_SECRET_LEN) {
        throw new Error(`JWT secrets must be at least ${MIN_JWT_SECRET_LEN} characters when NODE_ENV=production or STRICT_ENV is set`);
    }
}
...
export const env = {
     ...
     */
    payoutOptimizerEnabled: ['1', 'true', 'yes'].includes(String(process.env.PAYOUT_OPTIMIZER_ENABLED || '').toLowerCase()),
